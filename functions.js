...
// views department table when user selects it
...
//views role table when user selects is
function viewRole() {
  const role = [];
...
// views employee table when user selects it
function viewEmployee() {
  ...
  db.query(sql, (err, results) => {
    if (err) throw err;
    results.forEach((result) => {
      employee.push({ name: result.first_name, value: result.id });
    });
  });
  return employee;
}

// this adds the department
const addDepartment = () => {
...
// use this page to add the functionality of the user selections. then in the conditionals on index.js have it run a specific function depending on the users selection.